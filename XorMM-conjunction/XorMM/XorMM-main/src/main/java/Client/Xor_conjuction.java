...
public class Xor_conjuction {
    ...
    public static Element[] xtoken_flag;
    public static Element[] xtoken_key1;
    static byte[][] xitem;
    static byte[][] xitemone;
    static byte[][] EMM;
    static byte[][] XMM;
    public static String[] search_keys = new String[]{"key_s_27", "key_s_86"};
    //static byte[][] YMM;
    static byte[][] VMM;
   ...
   public class KChangePair {
        int k;
        int change;

        KChangePair(int k, int change) {
            this.k = k;
            this.change = change;
        }
    }
...
//    }
    ...
    public void XorMM_setup(KV[] kv_list, int level) throws Exception {
        ...
        xtoken_flag = new Element[kv_list.length];
...
//            byte[] K;
//            K = Hash.Get_Sha_128((K_f + kv_list[i].key).getBytes());
//            key_value[i] = AESUtil.encrypt(K, (kv_list[i].value + "," + kv_list[i].key).getBytes());
//             System.out.println("f: "+Arrays.toString(f[i]));
//        }
...
//                System.out.println("Generated secure tag: " + indexx + ":" + xtoken[indexx]);
        ...
        for (int u = 0; u < search_keys.length - 1; u++) {
            for (int o = 0; o < maxCounter; o++) {
                Element key_houmian = Hash.HashToZr(pairing, key_k[u]);
                Element key_value0 = Hash.HashToZr(pairing, xv_key1[xv_key1number]);
                Element xtokenelement = (key_houmian.mul(key_value0));
                xtoken_flag[indexxxx] = xtokenelement;
//                System.out.println("xtoken_flag: " + indexxxx + ":" + xtoken_flag[indexxxx]);
                indexxxx++;  // 每次存入后，索引递增
                xv_key1number++;
            }
        }
...
// 用于将二维数组的元素传输到一维数组
        ...
        for (int j = 0; j < kv_list.length; j++) {
            ...
            Element xtagelement = g.powZn(f_e.mul(xv_e));
            xtag[j] = xtagelement;
            // 存储或输出加密后的结果
//            System.out.println("xtag: " + xtag[j]);
        }
        Xor_conjuction.xtag = xtag;

        xtagitem_list = new xtagitem[kv_list.length];
// 现在可以将 y 数组的 Element 类型结果转换为 String

        for (int i = 0; i < Xor_conjuction.xtag.length; i++) {
            String xtagString = Xor_conjuction.xtag[i].toString();  // 将 Element 转换为 String
...
//                    System.out.println("EMM11: " +Arrays.toString(EMM[h]));
